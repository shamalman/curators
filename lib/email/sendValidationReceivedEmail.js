...
export async function sendValidationReceivedEmail({ validationId, supabaseAdmin }) {
  ...
  const { data: curator, error: curatorErr } = await sb
    .from('profiles')
    .select('id, name, handle, auth_user_id, feature_flags')
  ...
  const recSource = typeof rec.source === 'string' ? rec.source : (rec.source?.name || rec.source?.publisher || '');
  const recUrl = `https://curators.ai/${curator.handle}/${rec.slug}`;
