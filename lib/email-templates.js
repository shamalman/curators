// Curators email design system — single source of truth for all transactional email.
...
function emailWordmark() {
  return `
        <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
          <tr><td style="font-family:${SERIF};font-size:13px;color:${INK2};letter-spacing:0.08em;text-transform:uppercase;">curators.com</td></tr>
        </table>
        <div style="line-height:24px;font-size:0;">&nbsp;</div>`;
}

function emailFooter({ contextLine, unsubLabel = 'Unsubscribe', unsubUrl, settingsLabel = 'Notification settings', settingsUrl = undefined }) {
  const settings = settingsUrl || siteUrl('/settings');
  return `
        ...
        <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%">
          ...
          <tr><td style="font-family:${SANS};font-size:12px;color:${INK3};line-height:1.6;">
            <a href="${escapeHtml(unsubUrl)}" style="color:${INK3};text-decoration:underline;">${escapeHtml(unsubLabel)}</a> · <a href="${escapeHtml(settings)}" style="color:${INK3};text-decoration:underline;">${escapeHtml(settingsLabel)}</a>
          </td></tr>
        </table>`;
}
...
export function newSubscriberEmail({ subscriberName, subscriberHandle, subscriberCount, unsubscribeUrl }) {
  ...
  const profileLink = hasProfile
    ? `
        ...
        <p style="font-family:${SANS};font-size:14px;color:${INK2};line-height:1.6;margin:0;">
          ...
          <a href="${escapeHtml(siteUrl(`/${subscriberHandle}`))}" style="color:${ACCENT};text-decoration:none;">curators.com/${escapeHtml(subscriberHandle)}</a>
        </p>`
    : '';

  const subject = `${subscriberName} subscribed to you on Curators`;
  const preheader = `You now have ${subscriberCount} subscriber${subscriberCount === 1 ? '' : 's'} on Curators.`;
  ...
  const innerHtml = `
        ...
        ${emailFooter({
          contextLine: "You're receiving this because you're a curator on Curators.",
  ...
  const text = [
    `${subscriberName} just subscribed to you on Curators`,
    '',
    hasProfile ? `See what they recommend: ${siteUrl(`/${subscriberHandle}`)}` : '',
    hasProfile ? '' : null,
    `You now have ${subscriberCount} subscriber${subscriberCount === 1 ? '' : 's'}.`,
    '',
    '---',
    `Unsubscribe: ${unsubscribeUrl}`,
    `Notification settings: ${siteUrl('/settings')}`,
...
export function weeklyDigestEmail({ recs, subscribedCount, unsubscribeUrl }) {
  const rows = recs.map((rec, idx) => {
    ...
    const recUrl = siteUrl(`/${rec.curatorHandle}/${rec.slug}`);
  ...
  const preheader = `From curators you subscribe to on Curators.`;
  ...
  const innerHtml = `
        ...
        <div style="line-height:8px;font-size:0;">&nbsp;</div>
        <p style="font-family:${SANS};font-size:14px;color:${INK2};line-height:1.6;margin:0;">
          From curators you subscribe to on Curators.
        </p>
        <div style="line-height:12px;font-size:0;">&nbsp;</div>
        ${rows}
        ${emailFooter({
          contextLine: `You're receiving this because you subscribe to ${subscribedCount} curator${subscribedCount === 1 ? '' : 's'} on Curators.`,
          unsubUrl: unsubscribeUrl,
        })}`;
  ...
  recs.forEach(rec => {
    ...
    textLines.push(`  ${siteUrl(`/${rec.curatorHandle}/${rec.slug}`)}`);
    textLines.push('');
  });
  textLines.push('---');
  textLines.push(`Unsubscribe: ${unsubscribeUrl}`);
  textLines.push(`Notification settings: ${siteUrl('/settings')}`);
...
// ─────────────────────────────────────────────────────────────
// Template: Cosign received (subscriber → curator, money-attached note)
// ─────────────────────────────────────────────────────────────

export function validationReceivedEmail({
  curatorHandle,
  subscriberHandle,
  verbatimText,
  recTitle,
  ...
  const titleForSubject = recTitle
    ? (recTitle.length > 50 ? recTitle.slice(0, 50).trimEnd() + '…' : recTitle)
    : 'your recommendation';
  const subject = `@${subscriberHandle} cosigned ${titleForSubject}`;
  const preheader = makePreheader(verbatimText || `@${subscriberHandle} cosigned your recommendation.`, 90);

  const verbatimHtml = escapeHtml(verbatimText || '').replace(/\n/g, '<br/>');
  const aboutLine = `about: ${escapeHtml(recTitle || '(untitled)')}${recSource ? ` · ${escapeHtml(recSource)}` : ''}`;

  const earningsAmount = curatorEarnings ? parseFloat(curatorEarnings) : 0;
  const showEarnings = earningsAmount > 0;
  const earningsHtml = showEarnings ? `
        <div style="line-height:16px;font-size:0;">&nbsp;</div>
        <p style="font-family:${SANS};font-size:13px;color:${INK2};line-height:1.5;margin:0;">
          Your recommendations earned $${escapeHtml(curatorEarnings)} this month so far.
        </p>` : '';

  const innerHtml = `
        <p style="font-family:${SANS};font-size:15px;color:${INK2};line-height:1.5;margin:0;">
          Hi @${escapeHtml(curatorHandle)},
        </p>
        <div style="line-height:12px;font-size:0;">&nbsp;</div>
        <p style="font-family:${SANS};font-size:15px;color:${INK2};line-height:1.5;margin:0;">
          @${escapeHtml(subscriberHandle)} cosigned your recommendation.
        </p>
        ...
        <table role="presentation" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td style="font-family:${SERIF};font-style:italic;font-size:17px;line-height:1.55;color:${INK};">${verbatimHtml}</td>
          </tr>
        </table>

        <div style="line-height:20px;font-size:0;">&nbsp;</div>

        <p style="font-family:${SANS};font-size:13px;color:${INK3};line-height:1.5;margin:0;">
          ${aboutLine}
        </p>
        ...
        <table role="presentation" border="0" cellpadding="0" cellspacing="0">
          <tr><td>
            <a href="${escapeHtml(recUrl)}" style="font-family:${SANS};font-size:14px;font-weight:500;color:${INK};text-decoration:none;border-bottom:1px solid ${INK};padding-bottom:2px;">Reply in Curators →</a>
          </td></tr>
        </table>${earningsHtml}
        ${emailFooter({
          contextLine: `You're receiving this because @${escapeHtml(subscriberHandle)} cosigned one of your recommendations.`,
          unsubLabel: 'Manage notifications',
          unsubUrl: unsubscribeUrl,
        })}`;
  ...
  const text = [
    `Hi @${curatorHandle},`,
    '',
    `@${subscriberHandle} cosigned your recommendation.`,
    '',
    `"${verbatimText || ''}"`,
    '',
    `about: ${recTitle || '(untitled)'}${recSource ? ` · ${recSource}` : ''}`,
    '',
    `Reply in Curators: ${recUrl}`,
    showEarnings ? '' : null,
    showEarnings ? `Your recommendations earned $${curatorEarnings} this month so far.` : null,
    '',
    '---',
    `You're receiving this because @${subscriberHandle} cosigned one of your recommendations.`,
...
export function agentCompletionEmail({ curatorName, sourceType, unsubscribeUrl }) {
  ...
  const innerHtml = `
        ...
        <div style="line-height:24px;font-size:0;">&nbsp;</div>
        <a href="${siteUrl('/myai')}" style="font-family:${SANS};font-size:14px;font-weight:500;color:${INK};text-decoration:none;border-bottom:1px solid ${INK};padding-bottom:2px;">See what your AI found →</a>
        ${emailFooter({
          contextLine: "You're receiving this because you're a curator on Curators.",
          unsubUrl: unsubscribeUrl || siteUrl('/settings'),
        })}`;
  ...
  const text = [
    ...
    `I went through everything and found some interesting patterns in your taste. Come see what I found.`,
    '',
    `See what your AI found: ${siteUrl('/myai')}`,
    '',
    '---',
    `Unsubscribe: ${unsubscribeUrl || siteUrl('/settings')}`,
  ].join('\n');
...
${whyBlock}
        ...
        ${emailFooter({
          contextLine: `You get these because you subscribe to @${escapeHtml(curatorHandle)} on Curators.`,
  ...
  const textLines = [
    `${displayForName} sent you a recommendation on Curators`,
    '',
  ];
  ...
  textLines.push('');
  textLines.push('---');
  textLines.push(`You get these because you subscribe to @${curatorHandle} on Curators.`);
