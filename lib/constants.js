// ── Design-lab theming bridge ──
// Token values reference CSS custom properties with the current hex as the
// var() fallback. With no theme active (no --cp-* defined), every token
// resolves to its fallback = the exact production value. The dev-only
// ThemeProvider (components/theme/ThemeProvider.jsx) injects --cp-* overrides
// only under ?theme= / ?designlab=1. Default keys live in lib/themes/themes.js
// (theme id "paper-ink"). See docs note in that file before adding tokens.

// ── Theme: shared base ──
export const T = {
  bg: "var(--cp-t-bg,#131210)", bg2: "var(--cp-t-bg2,#1A1714)", s: "var(--cp-t-s,#201D18)", s2: "var(--cp-t-s2,#2A2620)", s3: "var(--cp-t-s3,#332F28)",
  ink: "var(--cp-t-ink,#E8E2D6)", ink2: "var(--cp-t-ink2,#A09888)", ink3: "var(--cp-t-ink3,#6B6258)",
  bdr: "var(--cp-t-bdr,#302B25)", acc: "var(--cp-t-acc,#D4956B)", accText: "var(--cp-t-accText,#131210)", accSoft: "var(--cp-t-accSoft,#D4956B20)",
};

// ── Curator workspace: cooler, blue-shifted ──
export const W = {
  bg: "var(--cp-w-bg,#101214)", s: "var(--cp-w-s,#181B20)", s2: "var(--cp-w-s2,#1F2329)", bdr: "var(--cp-w-bdr,#262B33)",
  aiBub: "var(--cp-w-aiBub,#181B20)", aiBdr: "var(--cp-w-aiBdr,#3B7BF6)", aiBdrSoft: "var(--cp-w-aiBdrSoft,#3B7BF620)",
  userBub: "var(--cp-w-userBub,#D4956B)", userTxt: "var(--cp-w-userTxt,#101214)",
  accent: "var(--cp-w-accent,#6B9EC2)", accentSoft: "var(--cp-w-accentSoft,#6B9EC220)",
  chip: "var(--cp-w-chip,#181B20)", chipBdr: "var(--cp-w-chipBdr,#262B33)",
  inputBg: "var(--cp-w-inputBg,#181B20)", inputBdr: "var(--cp-w-inputBdr,#262B33)",
};

// ── Visitor AI: warm, personal ──
export const V = {
  bg: "var(--cp-v-bg,#151310)", s: "var(--cp-v-s,#1E1A15)", bdr: "var(--cp-v-bdr,#302A22)",
  aiBub: "var(--cp-v-aiBub,#1E1A15)", userBub: "var(--cp-v-userBub,#A09888)", userTxt: "var(--cp-v-userTxt,#151310)",
  chip: "var(--cp-v-chip,#1E1A15)", chipBdr: "var(--cp-v-chipBdr,#302A22)",
  inputBg: "var(--cp-v-inputBg,#1E1A15)", inputBdr: "var(--cp-v-inputBdr,#302A22)",
};

// ── Fonts ──
export const F = "var(--cp-font-text,'Manrope',sans-serif)";
export const S = "var(--cp-font-display,'Newsreader',serif)";
export const MN = "var(--cp-font-mono,'JetBrains Mono',monospace)";
...
// ── Invite limits ──
// Per-curator cap on outstanding (pending) invite codes. Bypassed when
// profiles.unlimited_invites = true.
export const MAX_UNUSED_INVITES = 25;
...
export const CAT = {
  ...
  eat: { emoji: "\u{1F37D}", color: "#D4856B", bg: "#D4856B18", label: "Eat" },
  see: { emoji: "\u{1F441}", color: "#9DA88E", bg: "#9DA88E18", label: "See" },
...
// Canonical category list, derived from CAT so it can never drift.
export const CATEGORIES = Object.keys(CAT);
