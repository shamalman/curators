...
export async function generateTasteProfile(profileId, supabase) {
  ...
  const prompt = `You are generating a taste profile document for a curator on Curators.AI. This document is read by AI systems and human visitors to understand this curator's taste. It is an interpretation, not a verdict. The verbatim record of every recommendation and confirmation lives elsewhere on a separate timeline page; this document does not need to repeat that ledger.
...
${recentConfirmations.length > 0 ? `=== RECENT CONFIRMED OBSERVATIONS (last 90 days, weight these heavily) ===
${recentConfirmations.map(c => `- [${c.type}] (${c.created_at})\n"${c.observation}"`).join('\n\n')}
=== END RECENT ===` : ''}

${olderConfirmations.length > 0 ? `=== LONG-STANDING CONFIRMED OBSERVATIONS (older than 90 days, weight as background) ===
${olderConfirmations.map(c => `- [${c.type}] (${c.created_at})\n"${c.observation}"`).join('\n\n')}
=== END LONG-STANDING ===` : ''}

These observations are the curator's endorsed signals. The full verbatim record lives on a separate timeline page; you do NOT need to list them here. Use them as evidence to inform the sections below. When recent and long-standing observations are in tension, prefer the recent. When they reinforce each other, treat that as a stable pattern.
